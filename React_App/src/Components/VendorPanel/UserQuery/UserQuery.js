...
export const UsersQuery = () => {
  const [queryList, setQueryList] = useState([]);
    ...
    useEffect(()=>{
        fetch('http://localhost:5276/api/Vendor/GetQueries',{
            headers:{
    ...
    const QueryReplied=(qry,reply)=>{
        ...
        fetch('http://localhost:5276/api/Vendor/ReplyQuery',{
            method:'PUT',
            headers:{
    ...
    <table className="table table-striped">
        ...
        <tbody>
            {queryList.map(q=><QueryModal key={q.queryId} Query={q}  QueryReplied={QueryReplied}/>)}
        </tbody>
    </table>
