...
const QueryCard=({ Query, QueryReplied})=>{
    ...
    useEffect(()=>{
        fetch('http://localhost:5276/api/Admin/GetUserById/'+Query.queryUserOrVedId,{
            headers:{
                Authorization:`Bearer ${localStorage.getItem('token')}`
            }
        }).then(response=>response.json()).then(data=>setUserName(data.firstName+" "+data.lastName))
    },[]);
    return(<tr>
